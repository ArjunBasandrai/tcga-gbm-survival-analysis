...
def _cox_regression_preprocess(df_for_survival: pd.DataFrame) -> pd.DataFrame:
    ...
    df_for_survival['PC5'] = (df_for_survival['PC5'] - df_for_survival['PC5'].mean()) ** 2
    df_for_survival['PC13'] = (df_for_survival['PC13'] - df_for_survival['PC13'].mean()) ** 2
    df_for_survival['PC36'] = (df_for_survival['PC36'] - df_for_survival['PC36'].mean()) ** 2
...
def _plot_kaplan_meier_curves(df: pd.DataFrame, important_mirna: np.array, output_dir: str, p_value_threshold: float = 0.05) -> pd.DataFrame:
    ...
    pbar = tqdm(df_mirna.columns[0:-2])
    for mirna in pbar:
        pbar.set_description(f"Processing {mirna}")
        ...
        if (results_low_medium.p_value < p_value_threshold or
            ...
            plot_filename = os.path.join(output_dir, f"{mirna}_km_curve.png")
